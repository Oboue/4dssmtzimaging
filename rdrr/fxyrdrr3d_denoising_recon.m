function [ D1 ] = fxyrdrr3d_denoising_recon(D,MASK,flow,fhigh,dt,N,K,Niter,eps,verb,mode,a,h,e,ws)
%  FXYRDMSSA_RECON: F-XY domain robust damped multichannel singular spectrum analysis (DMSSA) for
%  known as robust damped rank-reduction method for simultaneous denoising and reconstruction
%
%  IN   D:   	 intput 3D data
...
function [dout]=P_RD(din,N,K,h,e,ws)
...
% %      
    ...
    dout=STMA(U(:,1:N)*D(1:N,1:N)*(V(:,1:N)'),h,e,ws);
% save dout.mat dout
% [l,m,n]=svd (dout);
% % m;
% save m.mat m
